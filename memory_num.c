#include "main.h"

unsigned int _memcpy(buffer_v *output, const char *src, unsigned int n);
void free_buffer(buffer_v *output);
buffer_v *init_buffer(void);

/**
 * _memcpy - Copies n bytes from memory area src to
 *           the buffer kept in a buffer_v struct.
 * @output: A buffer_v struct.
 * @src: A pointer to the memory area to copy.
 * @n: The number of bytes to be copied.
 *
 * Return:A number of bytes copied.
 */
unsigned int _memcpy(buffer_v *output, const char *src, unsigned int n)
{
	unsigned int index;

	for (index = 0; index < n; index++)
	{
		*(output->buffer) = *(src + index);
			(output->len)++;

			if (output->len == 1024)
			{
				write(1, output->start, output->len);
				output->buffer = output->start;
				output->len = 0;
			}

			else
				(output->buffer)++;
	}

	return (n);
}

/**
 * free_buffer - Frees the buffer_v struct.
 * @output: The buffer_v struct to be freed.
 */
void free_buffer(buffer_v *output)
{
	free(output->start);
	free(output);
}

/**
 * init_buffer - Initializes a variable of struct type buffer_v.
 *
 * Return: A pointer to initialized buffer_v.
 */
buffer_v *init_buffer(void)
{
	buffer_v *output;

	output = malloc(sizeof(buffer_v));
	if (output == NULL)
		return (NULL);

	output->buffer = malloc(sizeof(char) * 1024);
	if (output->buffer == NULL)
	{
		free(output);
		return (NULL);
	}

	output->start = output->buffer;
	output->len = 0;

	return (output);
}
